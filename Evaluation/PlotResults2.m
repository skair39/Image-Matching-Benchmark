...
mkdir('./Curves2');
CurveNames = {'./Curves2/r2_tum_video_',...
    './Curves2/r2_tum_wide_'};

Datasets = cell(2,1);
Datasets{1} = {[wkdir 'Dataset/01-office/'], ...
    [wkdir 'Dataset/02-teddy/'],...
    [wkdir 'Dataset/03-large-cabinet/']};
Datasets{2} = {[wkdir 'Dataset/06-office-wide/'], ...
...
for row = 1 : 2
    ...
    for idx = 1 : length(Dataset)
        [rSP, tSP, rAP, tAP, num] = Evaluate( Dataset{idx},Features,Matching,AngleThreshold, 2);
        rSPs = rSPs + rSP;
        tSPs = tSPs + tSP;
        rAPs = rAPs + rAP;
        tAPs = tAPs + tAP;
