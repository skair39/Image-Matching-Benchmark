...
int main(int argc, char **argv){
	...
	if(argc != 4)
    {
        cerr << endl << "Usage: ./RunAlgorithms2 FeatureName MethodName DatasetDir" << endl;
        cerr << "FeatureName : sift, surf, orb, akaze" << endl;
    ...
    // write results
    string ResultsDir = string(argv[3]) + "/Results2";
#ifdef __linux__
...
int MatchImages(Mat img1, Mat img2, string features, string methods, Mat K, Mat &pose){
    ...
    {
        extractor = xfeatures2d::SIFT::create(0,3,0.004,10);
        matcher = FlannBasedMatcher::create();
    }
    if (features == string("surf"))
    {
        extractor = xfeatures2d::SURF::create(10);
        matcher = FlannBasedMatcher::create();
    }
    if (features == string("orb"))
    {
        extractor = ORB::create(100000,1.2f,8,31,0,2,ORB::HARRIS_SCORE,31,2);
        matcher = BFMatcher::create(NORM_HAMMING);
    }
    if (features == string("akaze"))
    {
        extractor = AKAZE::create(AKAZE::DESCRIPTOR_MLDB,0,3,0.0001);
        matcher = BFMatcher::create(NORM_HAMMING);
    }
    if (features == string("kaze")){
        extractor = KAZE::create(false,false,0.0001f);
        matcher = FlannBasedMatcher::create();
    }
    if (features == string("brisk")){
        extractor = BRISK::create(3);
        matcher = BFMatcher::create(NORM_HAMMING);
    }
